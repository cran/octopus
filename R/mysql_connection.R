...
get_schemas_mysql <- function(con) {
  schemas <-
    DBI::dbGetQuery(
      con,
      "
...
get_tables_mysql <- function(con, schema) {
  result <- tryCatch({
    DBI::dbGetQuery(
      con,
      glue::glue(
...
get_preview_mysql <- function(con, schema, table) {
  DBI::dbGetQuery(
    con,
    glue::glue(
      "
...
delete_table_mysql <- function(con, schema, table){
